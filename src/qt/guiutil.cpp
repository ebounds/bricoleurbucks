...
{
    // return if URI is not valid or is no bricoleur: URI
    if(!uri.isValid() || uri.scheme() != QString("bricoleur"))
        return false;
...
{
    // Convert bricoleur:// to clam:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("bricoleur://"))
    {
        uri.replace(0, 12, "bricoleur:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Bricoleur.lnk";
}
...
{
    return GetAutostartDir() / "bricoleur.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Bricoleur-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  bricoleur-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("Bricoleur-Qt"));
