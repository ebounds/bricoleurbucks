...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool fPrintTransactionDetail)
{
    ...
    UniValue bricoleurours(UniValue::VARR);
    BOOST_FOREACH (const CBricoleurour& bricoleurour, blockindex->vClamour)
    {
        ...
        bricoleurours.push_back(entry);
    }
    if (bricoleurours.size())
        result.push_back(Pair("bricoleurours", clamours));
