...
UniValue getworkex(const UniValue& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Bricoleur is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Bricoleur is downloading blocks...");
...
UniValue getwork(const UniValue& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Bricoleur is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Bricoleur is downloading blocks...");
